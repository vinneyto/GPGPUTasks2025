...
void run(int argc, char** argv)
{
    ...
    gpu::gpu_mem_32u values_a_gpu(n), values_b_gpu(n);
    ...
    const unsigned int bits_per_pass = 2;
    const unsigned int passes = 32 / bits_per_pass;
    const unsigned int num_blocks = (n + block_size - 1) / block_size;
    const unsigned int num_chunks = (num_blocks + block_size - 1) / block_size; // ceil(num_blocks / 512)

    // Packed uint4 arrays: store as uint[4 * len]
    gpu::gpu_mem_32u counts4_gpu(num_blocks * 4);
    gpu::gpu_mem_32u scan_counts4_gpu(num_blocks * 4);
    gpu::gpu_mem_32u sums1_4_gpu(num_chunks * 4);
    gpu::gpu_mem_32u scan_sums1_4_gpu(num_chunks * 4);
    gpu::gpu_mem_32u dummy_sums4_gpu(4); // one uint4

    // Запускаем кернел (несколько раз и с замером времени выполнения)
    std::vector<double> times;
    for (int iter = 0; iter < 10; ++iter) { // TODO при отладке запускайте одну итерацию
        // Reset working buffer from input (device->device copy), keep input intact
        input_gpu.copyTo(values_a_gpu, input_gpu.size());
        ...
        if (context.type() == gpu::Context::TypeOpenCL) {
            gpu::gpu_mem_32u* in = &values_a_gpu;
            gpu::gpu_mem_32u* out = &values_b_gpu;

            for (unsigned int pass = 0; pass < passes; ++pass) {
                const unsigned int shift = pass * bits_per_pass;

                // 1) local counting: values -> counts4
                {
                    gpu::WorkSize ws(GROUP_SIZE, (size_t)num_blocks * GROUP_SIZE);
                    ocl_radixSort01LocalCounting.exec(ws, *in, counts4_gpu, (unsigned int)n, shift);
                }

                // 2) scan counts4 over blocks (hierarchical, 2 levels for this n)
                {
                    // level0: len=num_blocks
                    {
                        gpu::WorkSize ws(GROUP_SIZE, (size_t)num_chunks * GROUP_SIZE);
                        ocl_radixSort02GlobalPrefixesScanSumReduction.exec(ws, counts4_gpu, scan_counts4_gpu, sums1_4_gpu, num_blocks);
                    }
                    // level1(top): len=num_chunks (<=512)
                    {
                        gpu::WorkSize ws(GROUP_SIZE, GROUP_SIZE); // one group
                        ocl_radixSort02GlobalPrefixesScanSumReduction.exec(ws, sums1_4_gpu, scan_sums1_4_gpu, dummy_sums4_gpu, num_chunks);
                    }
                    // down-pass only for level0
                    {
                        gpu::WorkSize ws(GROUP_SIZE, (size_t)num_chunks * GROUP_SIZE);
                        ocl_radixSort03GlobalPrefixesScanAccumulation.exec(ws, scan_counts4_gpu, scan_sums1_4_gpu, num_blocks, block_size);
                    }
                }

                // 3) scatter: values + scan_counts4 -> out
                {
                    gpu::WorkSize ws(GROUP_SIZE, (size_t)num_blocks * GROUP_SIZE);
                    ocl_radixSort04Scatter.exec(ws, *in, scan_counts4_gpu, *out, (unsigned int)n, shift);
                }

                std::swap(in, out);
            }

            // passes is even (16), so result ends up in values_a_gpu
            in->copyTo(buffer_output_gpu, buffer_output_gpu.size());
